...
struct CsrImage {
    int startIdx;  // equal to total nnz of all previous images
    int width;
    int height;
    int flags;
    /*int channels; // currently only one chanell. multi-channel should be csr of csr.
    int r_channel; //padding out 128 bits needed before float vec4, and useful for many channel arrays
    int b_channel;
    int g_channel;*/
    vec4 rect; // 4 float representing position on triangle
...
#define CSR_FLAG_HW 8
#define CSR_FLAG_WH 0
#define CSR_FLAG_RGB 0
#define CSR_FLAG_BGR 1
#define CSR_FLAG_CSR 0
#define CSR_FLAG_CSC 2  // unsupported for now
#define CSR_FLAG_SPLIT 0
#define CSR_FLAG_INTERLEAVED 4

layout(std430, binding = 8) buffer CsrData {
    int levels;
    int total_nnz;
    CsrImage texLevels[];
};

layout(std430, binding = 9) buffer CsrPointers {
    int csr_pointers[];
};

layout(std430, binding = 10) buffer CsrIndVals {
    int csr_ind_vals[];
};
...
#define CSR_CSR_FLAG_HW 8
#define CSR_CSR_FLAG_WH 0
#define CSR_CSR_FLAG_RGB 0
#define CSR_CSR_FLAG_BGR 1
#define CSR_CSR_FLAG_CSR 0
#define CSR_CSR_FLAG_CSC 2  // unsupported for now
#define CSR_CSR_FLAG_SPLIT 0
#define CSR_CSR_FLAG_INTERLEAVED 4

layout(std430, binding = 11) buffer CsrData {
    int levels;
    int total_nnz;
    CsrImage texLevels[];
};

layout(std430, binding = 12) buffer CsrPointers {
    int csr_pointers[];
};

layout(std430, binding = 13) buffer CsrIndVals {
    int csr_ind_vals[];
};

#define GET_CSR_VALUE(i, interleaved) interleaved ? csr_ind_vals[i+1]:
